					...
					<ul class="nav pull-right">					
						...
						<li class="dropdown">
							...
							<ul class="dropdown-menu">
								...
								<li><a href="#"><i class="halflings-icon user"></i> 代理商名：<%=session.getAttribute("my_uid") %></a></li>
					...
					<form  action="ItemEdit" method="post" onsubmit="return check(this);">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">						
							...
							<tr>
								...
								<td class="center">
								<input type = "hidden" style="BACKGROUND-COLOR:transparent;" name = "oid" value="<%=p.getOid()%>" >
		            			<%=ordtime%>
								</td>							
							</tr>			
							<tr>
							...
							<tr>
								...
								<td class="center">
		            			<select id="selectError3" name="ostate">
									...
									<option value="1"  selected>待绑定</option>
									<option value="2" >使用中</option>
									<option value="3" >已结束</option>					            
							...
							<tr>
								...
								<td class="center">
								<% if (p.getOrderstate()>0){	 %>
								<input type = "text" name = "kdcompany" style="width:8rem;" value="<%=p.getKdcompany()%>">
								<% } else {	 %>
								<input type = "text" name = "kdcompany" style="width:8rem;" >
								<% } %>	
								</td>							
							</tr>
							<tr>
								...
								<td class="center">
								<% if (p.getOrderstate()>0){	 %>
								<input type = "text" name = "kdno" style="width:8rem;" value="<%=p.getKdno()%>">
								<% } else {	 %>
								<input type = "text" name = "kdno" style="width:8rem;" >
								<% } %>	
								</td>							
							</tr>								
							...
							<tr>
								...
								<td class="center">									
								<!--
								<input type = "text" name = "vid" style="width:8rem;" value="<%=p.getIno()%>">
								-->
								<%=p.getIno()%>
								</td>							
							</tr>
							<% } %>
								<!--
								<input type = "text" name = "vid" style="width:8rem;" >
								-->
							<!-- 							
							<tr>
								<td class="center">*租赁时长</td>
								<td class="center">
								<input type = "text" name = "renttime" style="width:8rem;" value="<%=p.getRenttime()%>">
								</td>							
							</tr>		
							<tr>
								...
								<td class="center">
								<% if (p.getOrderstate()>1){	 %>
								<input type="text" class="input-xlarge datepicker" name="starttime" value="<%=p.getStarttime()%>">							  
								<% } else {	 %>
								<input type = "text" name = "kdno" style="width:8rem;" >
								<% } %>	
								</td>							
